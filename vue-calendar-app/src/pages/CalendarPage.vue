...
const days = computed(() => {
  ...
  for (let day = 1; day <= last.getDate(); day++) {
    const currentMonthDay = new Date(date.getFull)
    array.push(new CalendarCell(new Date(date.getFullYear(), date.getMonth(), day), false, tasks.getTasksForDate(currentMonthDay)))
  }
  ...
  if (last.getDay() !== 0) {
    ...
    for (let i = 1; i < nextMonthDays; i++) {
      array.push(new CalendarCell(new Date(last.getFullYear(), last.getMonth(), last.getDate() + i), true, tasks.getTasksForDate(nextMonthDay)))
    }
  }
  return array
})
...
<template>
  ...
  <div class="week-header">
    <div v-for="day in ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']" :key="day" class="week-day">
      {{ day }}
    </div>
  </div>
  <div class="calendar-grid">
    <div v-for="day in days" :key="day.key" class="calendar-cell" :class="{ 'not-current-month': day.notCurrentMonth }">
